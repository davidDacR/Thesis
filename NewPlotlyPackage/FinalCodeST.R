############################################################
#Plotting PQI* of WBSs (X) with the defects found in ST (Y)#
############################################################
              ...
              if (sumC > 0) {    
                  ...
                  if (sumST > 0){
                    ...
                    sqlcmd_12 <- paste("select ifnull(count(*), 0) FROM defect_log_fact_hist d
                                        ...
                                        WHERE d.row_current_flag = 1 and pi.project_key  =  ", proj, " and pi.wbs_element_key = ", c, "and (ph.phase_name like 'System Test');");
...
p <- plot_ly( x = PQI_Componente, 
              y = Def_ST_KLOC,
              name="#Def(ST / KLOC) - PQI*",
              mode = "markers",
              type = "scatter",
              filename="ST_PQI")
layout(p, title = "#Def(ST / KLOC) - PQI*")
print(p)
...
# )
# 
# response <- py$plotly(data, kwargs=list(layout=layout, filename="SysTestAllRPQI", fileopt="overwrite"))
# url <- response$url
