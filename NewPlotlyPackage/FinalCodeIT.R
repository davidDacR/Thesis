                  ...
                  if (sumC > 0) {
                    ...
                    if (sumIT > 0){
                      ...
                      sqlcmd_12 <- paste("select ifnull(count(*), 0) FROM defect_log_fact_hist d
                                        ...
                                        WHERE d.row_current_flag = 1 and pi.project_key  =  ", proj, " and pi.wbs_element_key = ", c, "and (ph.phase_name like 'Build and Integration Test' or ph.phase_name like 'Integration Test');");
...
p <- plot_ly( x = PQI_Componente, 
              y = Def_IT_KLOC,
              name="#Def(IT / KLOC) - PQI*",
              mode = "markers",
              type = "scatter",
              filename="IT_PQI")
layout(p, title = "#Def(IT / KLOC) - PQI*")
print(p)
...
# )
# 
# response <- py$plotly(data, kwargs=list(layout=layout, filename="IntTestAllRPQI", fileopt="overwrite"))
# url <- response$url
