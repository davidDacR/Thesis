              ...
              sumC <- sumCAux[1,1];
                  ...
                  if (sumC > 0) {
                    ...
                    sqlcmd_11 <- paste("select ifnull(sum(tl.time_log_delta_minutes), 0) FROM time_log_fact_hist tl
                                 ...
                                 INNER JOIN wbs_element w ON w.wbs_element_key = pi.wbs_element_key
                                 INNER JOIN phase ph ON ph.phase_key = pi.phase_key
                                 WHERE tl.row_current_flag = 1 and pi.project_key  =  ", proj, " and pi.wbs_element_key = ", c, " and (ph.phase_name like 'Build and Integration Test' or ph.phase_name like 'Integration Test');");
                    sumITQ <- dbSendQuery(con, sqlcmd_11);
                    sumITAux <- dbFetch(sumITQ, n = -1);
                    sumIT <- sumITAux[1,1];
                    ...
                    if (sumIT > 0){
                      ...
                      sqlcmd_11 <- paste("select ifnull(sum(tl.time_log_delta_minutes), 0) FROM time_log_fact_hist tl
                                 ...
                                 INNER JOIN wbs_element w ON w.wbs_element_key = pi.wbs_element_key
                                 INNER JOIN phase ph ON ph.phase_key = pi.phase_key
                                 WHERE tl.row_current_flag = 1 and pi.project_key  =  ", proj, " and pi.wbs_element_key = ", c, " and (ph.phase_name like 'System Test');");
                      sumSTQ <- dbSendQuery(con, sqlcmd_11);
                      sumSTAux <- dbFetch(sumSTQ, n = -1);
                      sumST <- sumSTAux[1,1];
                      ...
                      if (sumST > 0){
                        ...
                        sqlcmd_12 <- paste("select ifnull(count(*), 0) FROM defect_log_fact_hist d
                                        ...
                                        WHERE d.row_current_flag = 1 and pi.project_key  =  ", proj, " and pi.wbs_element_key = ", c, "and (ph.phase_name like 'System Test' or ph.phase_name like 'Build and Integration Test' or ph.phase_name like 'Integration Test');");
...
PQI_Componente = vectorX
Def_ITST_KLOC = vectorY;
p <- plot_ly( x = PQI_Componente, 
              y = Def_ITST_KLOC,
              name="#Def(ITST / KLOC) - PQI*",
              mode = "markers",
              type = "scatter",
              filename="ITST_PQI")
layout(p, title = "#Def(ITST / KLOC) - PQI*")
print(p)
...
# )
# 
# response <- py$plotly(data, kwargs=list(layout=layout, filename="ITSTAllRPQITimesFilter", fileopt="overwrite"))
# url <- response$url
