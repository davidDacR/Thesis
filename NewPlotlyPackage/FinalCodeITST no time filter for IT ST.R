          ...
          if (sumDR > 0) {
            ...
            if (sumD > 0) {
              ...
              if (sumC > 0) {
                ...
                #total defects in  IT and ST
                sqlcmd_11 <- paste("select ifnull(count(*), 0) FROM defect_log_fact_hist d
                ...
                defQ <- dbSendQuery(con, sqlcmd_11);
...
Def_ITST_notime_KLOC = vectorY;
p <- plot_ly( x = PQI_Componente, 
              y = Def_ITST_notime_KLOC,
              name="#Def(ITST_notime / KLOC) - PQI*",
              mode = "markers",
              type = "scatter",
              filename="ITST_notime_PQI")
layout(p, title = "#Def(ITST_notime / KLOC) - PQI*")
print(p)
...
# )
# 
# response <- py$plotly(data, kwargs=list(layout=layout, filename="ITSTAllRPQI", fileopt="overwrite"))
# url <- response$url
